...
class Verification(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if(response["status"] == "ok"):
            ...
            await channel_dm.send(f'It appears that you are not verified with RoVer or our own database. A keyphrase has been sent to your dms. Please verify w/ ``?verify`` in our server.')
    ...
    async def verify(self, ctx, username : str = None, *, keyPhrase : str = None):
        ...
        if(username == None and keyPhrase == None):
            if(response["status"] == "ok"):

                # Change nickname (Error if attempting to change server owner's nickname)
                db.verificationEntry.insert(member.id, response["robloxUsername"], response["robloxId"], ctx.message.created_at)
                ...
                await channel.send(f'Here is your passphrase to verify. Please place this string of text (without quotes) "{temp_passphrase}" in your roblox profile blurb.')
                await ctx.send(f'It appears that you are not verified with RoVer or our own database. A keyphrase has been sent to your dms. Please reverify w/ ``?verify <username> <keyphrase>``')
    ...
    async def info(self, ctx, member: discord.Member):
        ...
        emb = discord.Embed()
        emb.set_author(name="Information", icon_url="https://cdn.discordapp.com/attachments/336577284322623499/683028692133216300/ac6e275e1f638f4e19af408d8440e1d1.png")
        emb.set_footer(text=f'\t\t\t\t\t\t\tTimestamp: {ctx.message.created_at}')
        emb.add_field(name=f'**{member.display_name}**', value=f'Discord Id: {query["discordid"]}\nUsername: {query["username"]}\nUser Id: {query["userid"]}\nDate: {query["date"]}', inline=False)
